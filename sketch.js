...
const rookmove = ['right','left','up','down'];
const bishopmove = ['northeast','northwest','southeast','southwest'];
const queenmove = rookmove.concat(bishopmove);


const moveset = [knightmove,pawnmove,kingmove,generalmove,camelmove,wildebeestmove,ferzmove,wazirmove,alfilmove,dabbabamove,alibabamove,rookmove,bishopmove,queenmove];
...
let speed = 5;
let rounds = 0;
let loopunit = [-1,1,-1,1,0]; // step no., steps remaining in this direction, step direction, direction vector (x,y)
let positions = Array.from({ length: numplayers }, () => [...[-1,0]]);
        ...
        for(let multiple=0;multiple<speed;multiple++) {
            for(let turn=0;turn<numplayers;turn++) {
                ...
                for(let i=0;i<moveset[players[turn]].length;i++) { // attacking
                    if(typeof moveset[players[turn]][i] == 'string') {
                        switch(moveset[players[turn]][i]) { //for infinite reaching pieces
                            case 'up':
                                for(let j=1;j+middle+ypos<=size-1;j++) {
                                    attackdictionary[middle+xpos][middle+ypos+j][turn]=true;
                                }
                                break;
                            case 'down':
                                for(let j=-1;j+middle+ypos>=0;j--) {
                                    attackdictionary[middle+xpos][middle+ypos+j][turn]=true;
                                }
                                break;
                            case 'right':
                                for(let j=1;j+middle+xpos<=size-1;j++) {
                                    attackdictionary[middle+xpos+j][middle+ypos][turn]=true;
                                }
                                break;
                            case 'left':
                                for(let j=-1;j+middle+xpos>=0;j--) {
                                    attackdictionary[middle+xpos+j][middle+ypos][turn]=true;
                                }
                                break;
                            case 'northeast':
                                for(let j=1, k=1;j+middle+xpos<=size-1 && k+middle+ypos<=size-1;j++,k++) {
                                    attackdictionary[middle+xpos+j][middle+ypos+k][turn]=true;
                                }
                                break;
                            case 'northwest':
                                for(let j=-1, k=1;j+middle+xpos>=0 && k+middle+ypos<=size-1;j--,k++) {
                                    attackdictionary[middle+xpos+j][middle+ypos+k][turn]=true;
                                }
                                break;
                            case 'southwest':
                                for(let j=-1, k=-1;j+middle+xpos>=0 && k+middle+ypos>=0;j--,k--) {
                                    attackdictionary[middle+xpos+j][middle+ypos+k][turn]=true;
                                }
                                break;
                            case 'southeast':
                                for(let j=+1, k=-1;j+middle+xpos<=size-1 && k+middle+ypos>=0;j++,k--) {
                                    attackdictionary[middle+xpos+j][middle+ypos+k][turn]=true;
                                }
                                break;
                        }
